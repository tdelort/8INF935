...
{
    float k;
    contactNormal = (particle[1]->position() - particle[0]->position()).normalize();
...
void ParticleContact::resolveInterpenetration(float duration)
{
    ...
    {   //TODO : mettre une getter sur la masse d'une particule
        if(particle[1]->velocity().norm() < 0.01 && particle[0]->velocity().norm() < 0.01)
            return;
        float ma = 1.0f / particle[0]->invMass();
        float mb = 1.0f / particle[1]->invMass();
        particle[1]->setPosition(particle[1]->position() + (-ma / (ma + mb) * penetration * contactNormal));
        particle[0]->setPosition(particle[0]->position() + ( mb / (ma + mb) * penetration * contactNormal));
    }
    else                // cas 2 : choc entre particule et surface immuable
    {
